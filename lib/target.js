...
class Target {
  ...
  configureForDarwin() {
    ...
    this.osx_dmg_label = this.osx_dmg_filename = `${this.truncatedProductName}.dmg`;
    this.osx_zip_label = this.osx_zip_filename = `${this.truncatedProductName}.zip`;

    this.assets = [
      {
        name: `${this.id}-${this.version}-${this.platform}-${this.arch}.dmg`,
        path: this.dest(`${this.truncatedProductName}.dmg`)
      },
      {
        name: `${this.id}-${this.version}-${this.platform}-${this.arch}.zip`,
        path: this.dest(`${this.truncatedProductName}.zip`)
      }
    ];

    this.installerOptions = {
      dmgPath: this.dest(`${this.truncatedProductName}.dmg`),
