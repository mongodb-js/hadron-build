...
class Target {
   ...
   */
  configureForWin32() {
    const platformSettings = _.get(this, 'config.hadron.build.win32', {});
    /**
     ...
     */
    this.windows_msi_label = this.windows_msi_filename = `${this.productName}Setup.exe`;
    this.windows_setup_label = this.windows_setup_filename = `${this.productName}Setup.msi`;
   ...
   */
  configureForDarwin() {
    const platformSettings = _.get(this, 'config.hadron.build.darwin', {
    ...
    // const OSX_RESOURCES = this.dest(`${this.productName}-darwin-x64`, `${this.productName}.app`, 'Contents', 'Resources');
   ...
   */
  configureForLinux() {
    const platformSettings = _.get(this, 'config.hadron.build.linux', {});
    ...
    this.resources = LINUX_RESOURCES;
    this.appPath = LINUX_OUT_X64;
    this.assets = [
      {
    ...
    const createRpmInstaller = cb => {
      which('rpmbuild', err => {
        ...
        createRpm({
          ...
          dest: this.out,
          arch: this.arch,
          icon: this.platformSettings.icon,
          name: this.slug,
