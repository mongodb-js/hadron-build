...
class Target {
  ...
  configureForWin32() {
     ...
     */
    this.windows_msi_label = this.windows_msi_filename = `${
      this.productName
    }Setup.msi`;
    this.windows_setup_label = this.windows_setup_filename = `${
      this.productName
    }Setup.exe`;
    this.windows_zip_label = this.windows_zip_filename = `${
      this.productName
    }-windows.zip`;
    this.windows_nupkg_full_label = this.windows_nupkg_full_filename = `${
      this.packagerOptions.name
    }-${nuggetVersion}-full.nupkg`;
  ...
  configureForLinux() {
    ...
    this.installerOptions = {
      deb: {
        ...
        depends: ['libsecret-1-0', 'libgconf-2-4']
      },
      rpm: {
        src: this.appPath,
        dest: this.out,
        ...
        bin: this.productName,
        requires: ['lsb-core-noarch', 'libXScrnSaver', 'libsecret', 'GConf2'],
