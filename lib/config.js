...
exports.get = (cli, callback) => {
  ...
  let PRODUCT_NAME = _.get(cli, 'argv.product_name', PKG.productName || name);
  assert(PRODUCT_NAME);
  ...
  CONFIG.platform = _.get(cli, 'argv.platform', process.platform);
  CONFIG.arch = _.get(cli, 'argv.arch', process.arch);
  ...
  if (CONFIG.platform === 'win32') {
    ...
    const LINUX_OUT_TAR = CONFIG.dest(`${CONFIG.slug}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.tar.gz`);
    ...
    CONFIG.assets = [
      {
        name: `${CONFIG.slug}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.tar.gz`,
        path: LINUX_OUT_TAR
      },
      {
        name: `${CONFIG.slug}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.deb`,
        path: LINUX_OUT_DEB
      },
      {
        name: `${CONFIG.slug}.${CONFIG.version}.${CONFIG.platform}.${CONFIG.arch}.rpm`,
        path: LINUX_OUT_RPM
      },
      {
        name: `${CONFIG.slug}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.zip`,
        path: LINUX_OUT_ZIP
      }
    ];
