...
exports.get = (cli, callback) => {
  ...
  if (cli.argv.platform === 'win32') {
     ...
     */
    const WINDOWS_APPNAME = PRODUCT_NAME.replace(/ /g, '');
    const WINDOWS_NUGET_ID = CONFIG.name.replace('-', '_');
    ...
    const WINDOWS_OUT_MSI = CONFIG.dest(`${PRODUCT_NAME}Setup.msi`);
    ...
    CONFIG.windows_nupkg_full_filename = `${WINDOWS_NUGET_ID}-${CONFIG.version}-full.nupkg`;
    CONFIG.windows_nupkg_full_label = `${WINDOWS_NUGET_ID}-${CONFIG.version}-full.nupkg`;
    ...
    CONFIG.installerOptions = {
      ...
      /**
       * TODO (imlucas) Figure out wtf these things do.
       */
     ...
     * ## OS X Configuration
     */
    const OSX_APPNAME = PRODUCT_NAME;
