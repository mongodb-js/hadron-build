...
exports.get = (cli, callback) => {
  ...
  if (cli.argv.platform === 'win32') {
     ...
     * ## Linux Configuration
     */
    const LINUX_OUT_X64 = CONFIG.dest(`${CONFIG.id}-linux-x64`);
    const LINUX_RESOURCES = path.join(LINUX_OUT_X64, 'resources');

    // const LINUX_ICON = CONFIG.src(_.get(PKG, 'config.hadron.build.linux.icon', `${CONFIG.id}.png`));

    const LINUX_OUT_DEB = CONFIG.dest(`${CONFIG.id}_${CONFIG.version}_${CONFIG.arch}.deb`);

    // const LINUX_OUT_RPM = CONFIG.dest(`${CONFIG.id}_${CONFIG.version}_${CONFIG.arch}.rpm`);
    //
    // const LINUX_OUT_TAR = CONFIG.dest(`${CONFIG.id}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.tar.gz`);

    const LINUX_OUT_ZIP = CONFIG.dest(`${CONFIG.id}.zip`);
    ...
    CONFIG.assets = [
      // {
      //   name: `${CONFIG.id}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.tar.gz`,
      //   path: LINUX_OUT_TAR
      // },
      {
        name: `${CONFIG.id}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.deb`,
        path: LINUX_OUT_DEB
      },
      // {
      //   name: `${CONFIG.id}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.rpm`,
      //   path: LINUX_OUT_RPM
      // },
      {
        name: `${CONFIG.id}-${CONFIG.version}-${CONFIG.platform}-${CONFIG.arch}.zip`,
